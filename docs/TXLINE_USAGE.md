...
| **The minted `OuBoundReceipt`** (TxLINE-anchored) | PROPCAST verifies the complete immutable binding: owner, discriminator, PDA, embedded market, fixture, line, and canonical outcome byte. | `src/onchain/settle_consumer.ts` (`verifyOuReceiptForMarket`), `src/onchain/receipt.ts`, re-verified in-browser by the fan in `ui/src/App.tsx` |
