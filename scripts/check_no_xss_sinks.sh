...
# raw HTML — this locks that invariant as the close-path grows. Fails (exit 1) if any tracked ui/src or src file
...
FILES=$(git ls-files 'ui/src/*' 'src/*' 2>/dev/null || true)
[ -z "$FILES" ] && FILES=$(find ui/src src -type f 2>/dev/null)
...
echo "✅ XSS-sink guard passed (no HTML-injection sinks in ui/src or src)."
