...
It runs (~10s, deterministic): `build` (tsc) → `test` (128 tests) → `cleanroom` → `doc-drift` → `xss-guard`.
...
- `Tests 128 passed` — the factory + line-bound settle-consumer + the in-browser re-verify.
