...
It runs (~10s, deterministic): `build` (tsc) → `test` (126 tests) → `cleanroom` → `doc-drift` → `xss-guard`.
...
- `Tests 126 passed` — the factory + line-bound settle-consumer + the in-browser re-verify.
